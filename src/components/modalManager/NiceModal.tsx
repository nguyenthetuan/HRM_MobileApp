...
enum NiceModalActionType {
  show = 'nice-modal/show',
  hide = 'nice-modal/hide',
  setFlags = 'nice-modal/setFlags',
  remove = 'nice-modal/remove',
  removeAll = 'nice-modal/removeAll',
}

export interface NiceModalAction {
  type: NiceModalActionType
  payload?: {
    modalId: string
    args?: Record<string, unknown>
...
export const reducer = (
  ...
  switch (action.type) {
    ...
    case NiceModalActionType.remove: {
      const { modalId } = action.payload
      const newState = { ...state }
      delete newState[modalId]
      return newState
    }
    case NiceModalActionType.setFlags: {
    ...
    case NiceModalActionType.removeAll: {
      return {}
    }
    default:
      return state
...
function setModalFlags(
  ...
  return {
    type: NiceModalActionType.setFlags,
    payload: {
      modalId,
      flags,
    },
...
function removeModal(modalId: string): NiceModalAction {
  return {
    type: NiceModalActionType.remove,
    payload: {
      modalId,
    },
  }
...
// action creator to remove all modal
function removeAllModal(): NiceModalAction {
  return {
    type: NiceModalActionType.removeAll,
  }
}
...
export function show(
  ...
  console.log('show modalId', modalId)
...
export function hide(modal: string | React.FC<any>) {
  const modalId = getModalId(modal)
  console.log('hide modalId', modalId)
...
export const removeAll = (): void => {
  ...
  Object.keys(modalCallbacks).forEach((key) => {
    delete modalCallbacks[key]
  })
  Object.keys(hideModalCallbacks).forEach((key) => {
    delete modalCallbacks[key]
  })
}
...
export const withNiceModal = (Component: React.FC<any>) => {
  const ModalContainer = (props) => {
    const modal = useModal()

    return <Component visible={modal.visible} {...props} />
  }
  return NiceModal.create(ModalContainer)
}
